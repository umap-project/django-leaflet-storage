...
urlpatterns += decorated_patterns('', [map_permissions_check, never_cache, ],
    ...
    url(r'^map/(?P<map_id>[\d]+)/datalayer/add/$', views.DataLayerCreate.as_view(), name='datalayer_add'),
    url(r'^map/(?P<map_id>[\d]+)/datalayer/edit/(?P<pk>\d+)/$', views.DataLayerUpdate.as_view(), name='datalayer_update'),
